import { useState, useRef } from 'react';
import { useTranslation } from 'react-i18next';

export default function UploadingComp2({ 
  frontImage, setFrontImage,
  backImage, setBackImage,
}) {
  const [frontPreview, setFrontPreview] = useState(null);
  const [backPreview, setBackPreview]   = useState(null);
const {t} = useTranslation()
  const frontInputRef = useRef(null);
  const backInputRef  = useRef(null);

  const handleFrontImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setFrontImage(file);
      setFrontPreview(URL.createObjectURL(file));
    }
  };

  const handleBackImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setBackImage(file);
      setBackPreview(URL.createObjectURL(file));
    }
  };

  const handleRemoveFront = () => {
    setFrontImage(null);
    setFrontPreview(null);
    if (frontInputRef.current) frontInputRef.current.value = '';
  };

  const handleRemoveBack = () => {
    setBackImage(null);
    setBackPreview(null);
    if (backInputRef.current) backInputRef.current.value = '';
  };

  const UploadSlot = ({ label, preview, inputRef, onChange, onRemove }) => (
    <main className="relative z-10 w-full max-w-md">
      <label className="block text-sm font-semibold text-white mb-3">
        {label} <span className="text-red-400">*</span>
      </label>

      {preview ? (
        <div className="relative">
          <img
            src={preview}
            alt={label}
            className="w-full h-64 object-cover rounded-lg border-2 border-gray-300"
          />
          <button
            onClick={onRemove}
            className="absolute bottom-2 right-2 bg-red-500 hover:bg-red-600 text-white p-2 rounded-full transition duration-200 shadow-lg"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"
              />
            </svg>
          </button>
        </div>
      ) : (
        <label className="group relative block cursor-pointer rounded-lg border-2 border-dashed border-gray-300 bg-white/95 p-8 hover:shadow-lg transition-shadow duration-200">
          <input
            ref={inputRef}
            type="file"
            accept="image/png, image/jpeg, image/jpg, image/webp"
            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
            onChange={onChange}
          />
          <div className="flex flex-col items-center gap-4 text-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-14 w-14 text-gray-500"
              fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}
            >
              <path strokeLinecap="round" strokeLinejoin="round"
                d="M3 15a4 4 0 004 4h10a4 4 0 004-4 4 4 0 00-4-4H7a4 4 0 00-4 4z"
              />
              <path strokeLinecap="round" strokeLinejoin="round"
                d="M7 10l5-5m0 0l5 5m-5-5v12"
              />
            </svg>
            <div className="text-lg font-medium text-gray-700 group-hover:text-gray-900">
              Rasm yuklang
            </div>
            <p className="text-sm text-gray-400">
              PNG, JPG, JPEG, WEBP ruxsat etilgan.
            </p>
          </div>
          <span className="pointer-events-none absolute inset-0 rounded-lg ring-2 ring-transparent group-focus-within:ring-blue-300" />
        </label>
      )}
    </main>
  );

  return (
    <div className="flex md:flex-row items-center justify-center bg-gray-900 gap-4 p-4">
      <UploadSlot
        label={t("passportold")}
        preview={frontPreview}
        inputRef={frontInputRef}
        onChange={handleFrontImageChange}
        onRemove={handleRemoveFront}
      />
      <UploadSlot
        label={t("passportorqa")}
        preview={backPreview}
        inputRef={backInputRef}
        onChange={handleBackImageChange}
        onRemove={handleRemoveBack}
      />
    </div>
  );
}